...
void get_ms(unsigned long *count)
{
	...
	if( 0 != count )
	{
		*count = 0;
		...
		{
			*count = 1000*tp.tv_sec + tp.tv_nsec/(1000*1000);
		}
	}
}
#define labs        abs
...
// detect pi board revision
static int __GetBoardRev()
{
	FILE *fd;
	int iRev = -1;
	char szBuff[128];

	if( NULL != (fd=fopen( "/proc/cpuinfo", "r" )) )
	{
		int iFound = 0;
		int len;

		for(;NULL!=fgets(szBuff, 128, fd);)
		{
			if( strncmp( szBuff, "Revision", 8 ) == 0 )
			{
				iFound = 1;
				break;
			}
		...
		{
			len = strlen(szBuff);
			while( len && (szBuff[len-1] == '\r' || szBuff[len-1] == '\n') )
			{
				--len;
			}

			if( '2' == szBuff[len-1] || '3' == szBuff[len-1] )
				iRev = 1;
			else
				iRev = 2;
		}
...
static int __OpenDevice()
{
	...
	if( -1 == iRev )
	{
		printf("Can't detect Raspberry Pi board revision\n");
		return -1;
	}
...
//	int tries, result, total;
	...
	else
	{
		for(;length>0;)
		{   // for FIFO read ?
			iReadLen = min(__s_iMaxProcessLen,length);
			if( iReadLen != i2c_smbus_read_i2c_block_data( _s_fd, reg_addr, iReadLen, data ) )
			{
...
#define LOAD_CHUNK  (16)
	...
	{
		printf("DMP should only be loaded once\n");
    ...
    for (ii = 0; ii < length; ii += this_write) {
		...
		{
			printf("mpu_write_mem failed, this_write=%d\n", this_write);
            return -1;
		}
        if (mpu_read_mem(ii, this_write, cur))
		{
			printf("mpu_read_mem failed\n");
            return -1;
		}
        if (memcmp(firmware+ii, cur, this_write))
		{
			printf("memcmp not return 0, ii=%d\n", ii);
			int _i;
			printf("read:\n");
			for(_i=0;_i<this_write;_i++)
			{
				if( 0!=_i && 0==_i%8 )
					printf("\n");
				printf("0x%02x, ", cur[_i]);
			}
			printf("\n");
...
// p_iBitLen		new bit value bit len
// return£ºnew byte value
static inline unsigned char __SetBitValue( unsigned char p_ucOrg, unsigned char p_ucNew, int p_iBitBegin, int p_iBitLen )
{
	static const unsigned char __mask[] = {0,1,3,7,15,31,63,127,255};

	++p_iBitBegin;

#ifdef __CHECK_INNER_FUNCTION_ARGS
	if( p_iBitBegin > 8 || p_iBitLen > p_iBitBegin )
	{
		throw "error arg";
	}
	else
	{
#endif
		return ((((p_ucOrg>>p_iBitBegin)<<p_iBitLen)|p_ucNew)<<(p_iBitBegin-p_iBitLen))|(p_ucOrg&__mask[p_iBitBegin-p_iBitLen]);
#ifdef __CHECK_INNER_FUNCTION_ARGS
	}
#endif
}
...
{
	if( 0 != i2c_read(st.hw->addr, st.reg->who_am_i, 1, p_pData) )
	{
		return -1;
	}
	else
...
int __mpu_get_temperature( float *p_fTemperature )
{
	...
	*p_fTemperature = 35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens);
...
{
	if( 0 != i2c_read(st.hw->addr, st.reg->pwr_mgmt_1, 1, p_pData) )
	{
		return -1;
	}
	else
...
int __mpu_set_sleep_mode( unsigned char p_ucVal )
{
	...
	p_ucVal = (0==p_ucVal)?0:1;

	if( 0 != i2c_read(st.hw->addr, st.reg->pwr_mgmt_1, 1, &ucOrg) )
	{
		return -1;
	}

	ucOrg = __SetBitValue( ucOrg, p_ucVal, 6, 1 );

	if( 0 != i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &ucOrg) )
	{
		return -1;
	}
	else
...
int __mpu_get_fifo_bytes_count( unsigned short *p_usCount )
{
	...
	if( 0 != i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data) )
	{
		return -1;
	}

	*p_usCount = (data[0] << 8) | data[1];
...
int __mpu_drop_fifo_data( int len )
{
	...
	if( len <= 0 || len > 1024 || 0 != i2c_read(st.hw->addr, st.reg->fifo_r_w, len, data) )
		return -1;
	else
		return 0;
}
